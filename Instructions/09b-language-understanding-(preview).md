...
6. When the model has been deployed, on the **Test model** page, select the **Clock** model.

6. On the **Test Model: Clock** page, in the **Deployment name** dropdown, select **production**.  
...
2. Select the **View model details** page and then select the **Clock** model. Review the overall, per-entity, and per-intent evaluation metrics (*precision*, *recall*, and *F1 score*) and the *confusion matrix* generated by the evaluation that was performed when training (note that due to the small number of sample utterances, not all intents may be included in the results).

3. On the **Deploy model** page, select **Add deployment**.
...
4. When the model is deployed, on the **Test model** page, select the **Clock** model, select the **production** deployment, and then test it with the following text:
